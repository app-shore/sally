...
**Category:** Intelligent TMS & Fleet Operations Platform (integrates with or replaces existing TMS + ELD)
...
**Core Value:**
- **For Dispatchers:** Automated route planning with HOS validation, proactive alerts when intervention needed, real-time monitoring of all active routes, load management, invoicing, and driver settlements
- **For Drivers:** Clear turn-by-turn guidance with rest/fuel stops inserted, automatic plan updates as conditions change

**Target Market:**
* Small-to-mid carriers (20–100 trucks, sweet spot)
* Currently using basic TMS (Truckbase, Rose Rocket) + ELD (Samsara, Motive)
* Pain: Manual route planning, no HOS-aware dispatch, constant dispatcher-driver communication, TMS that only tracks but doesn't plan
...
* "HOS-compliant, fuel-optimized, continuously monitored — no other TMS does this."
* "Keep your current TMS and add SALLY's intelligence, or switch entirely — your choice."
* "Samsara tracks hours. Your TMS tracks loads. **SALLY plans the route nobody else can.**"
...
- TMS (load management, dispatch, invoicing, settlements)
- ELD (HOS tracking)
- Routing (directions)
...
- AI-powered nerve center (voice + text) for dispatchers and drivers

**Category-creating, not feature-competing.**

### SALLY vs. Competitors (February 2026)

| Capability | Truckbase | Rose Rocket | Trimble | **SALLY** |
|-----------|-----------|-------------|---------|-----------|
| Load/dispatch management | Yes | Yes | Yes | **Yes** |
| Invoicing | Yes | Yes | Yes | **Phase 1** |
| Driver settlements | Yes | Yes | Yes | **Phase 1** |
| QuickBooks integration | Yes | Yes | Yes | **Phase 1** |
...
| **AI assistant (voice + text)** | No | No | No | **Yes** |
| **Dynamic route re-planning** | No | No | Partial | **Yes** |
| Target fleet size | 10-50 | 10-500 | 50-5000 | **20-100** |
| Pricing | $290-490/mo | ~$50/user/mo | $1,000+/mo | **TBD** |

**Key insight:** No TMS for small carriers (20-100 trucks) combines dispatch operations with intelligent, HOS-aware route planning. The only competitor doing something similar (Trimble) targets enterprise carriers at $1,000+/month. SALLY fills this gap.

### TMS Strategy: Dual-Source Data Model
...
2. **Manual Entry** — for carriers with no TMS, or TMS without APIs (Truckbase)

Both paths feed into the same route planning and monitoring intelligence. For TMS loads, SALLY syncs operational status back to the source TMS. For manual loads, SALLY is the source of truth.

**TMS Roadmap:**
- **Phase 0:** Enable manual entry for drivers, vehicles, loads + load lifecycle
...
See `.docs/plans/2026-02-09-sally-tms-strategy-and-phase0-design.md` for detailed design.
