...
```bash
curl -X POST https://sally-api.apps.appshore.in/api/v1/route-planning/optimize \
...
- `balanced`: Balance time and cost (recommended)
...
- Valid enum values for `optimization_priority` (time, cost, balanced)
