...
1. **Material UI (MUI)** -- Comprehensive component library with theming support. Installed as an npm package. Customization requires overriding the theme object and dealing with CSS-in-JS specificity issues.
...
- New components are added with `npx shadcn@latest add [component-name]`.
