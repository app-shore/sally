...
description: Single repository with Turborepo and npm workspaces for shared types and coordinated builds.
---
...
1. **Multi-repo** -- Each application in its own repository with shared types published as an npm package.
...
Use a **Turborepo monorepo with npm workspaces**. The repository is structured as:
...
Turborepo manages the build pipeline. npm workspaces handle dependency resolution, allowing any app to import from `@sally/shared-types` without publishing to a registry.

Key configuration choices:

- **Turborepo** for task orchestration (build, lint, test, typecheck) with remote caching.
- **npm workspaces** (not Yarn or pnpm) for package management, keeping the toolchain simple.
