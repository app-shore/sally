...
- **Response Time:** Less than 5s for route planning (10 stops)
...
- **Response Time:** Less than 10s for fleet-wide optimization (50 drivers, 200 loads)
...
- `GET /api/v1/external/fuel-prices` - Mock fuel prices
- `GET /api/v1/external/weather` - Mock weather data

### Component Endpoints (Called by Route Planner)
- `POST /api/v1/hos/validate` - HOS compliance check
- `POST /api/v1/rest/recommend` - REST optimization
- `POST /api/v1/fuel/find-stops` - Fuel stop finding
