...
1. Make sure the backend is running (`npm run backend:dev` or `npm run dev`).
...
|------|---------|----------|
| Start all apps | `npm run dev` | Root |
| Start backend only | `npm run backend:dev` | Root |
| Start frontend only | `npm run frontend:dev` | Root |
| Start docs only | `npm run docs:dev` | Root |
| Start Docker (DB + Redis) | `npm run docker:up` | Root |
| Stop Docker | `npm run docker:down` | Root |
| Install dependencies | `npm install` | Root |
| Run migrations | `npm run prisma:migrate` | `apps/backend/` |
| Generate Prisma client | `npm run backend:prisma:generate` | Root |
| Open Prisma Studio | `npm run backend:prisma:studio` | Root |
| Seed base data | `npm run setup:base` | `apps/backend/` |
| Seed demo data | `npm run setup:demo` | `apps/backend/` |
| Reset database | `npm run setup:reset` | `apps/backend/` |
| Run backend tests | `npm run test` | `apps/backend/` |
| Run frontend tests | `npm run test` | `apps/web/` |
| Build all | `npm run build` | Root |
| Add Shadcn component | `npx shadcn@latest add [name]` | `apps/web/` |
