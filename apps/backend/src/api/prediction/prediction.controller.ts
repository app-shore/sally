...
export class PredictionController {
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Drive demand prediction' })
  async estimateDriveDemand(@Body() body: PredictionRequest) {
    this.logger.log(`Prediction requested for destination: ${body.destination}`);
    ...
    try {
      ...
      this.logger.log(`Prediction completed: ${result.estimated_drive_hours}h, high_demand=${result.is_high_demand}, low_demand=${result.is_low_demand}`);
      return result;
    } catch (error) {
