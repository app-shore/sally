...
const configSchema = z.object({
  ...
  secretKey: z
    .string()
    .default('sally-development-secret-key-minimum-32-chars'),

  // JWT Configuration
  jwt: z.object({
    accessSecret: z
      .string()
      .default('sally-jwt-access-secret-change-in-production-min-32-chars'),
    refreshSecret: z
      .string()
      .default('sally-jwt-refresh-secret-change-in-production-min-32-chars'),
...
export default (): Configuration => {
  const raw = {
    ...
    maxDriveHours: process.env.MAX_DRIVE_HOURS
      ? Number(process.env.MAX_DRIVE_HOURS)
      : undefined,
    maxDutyHours: process.env.MAX_DUTY_HOURS
      ? Number(process.env.MAX_DUTY_HOURS)
      : undefined,
    requiredBreakMinutes: process.env.REQUIRED_BREAK_MINUTES
      ? Number(process.env.REQUIRED_BREAK_MINUTES)
      : undefined,
    breakTriggerHours: process.env.BREAK_TRIGGER_HOURS
      ? Number(process.env.BREAK_TRIGGER_HOURS)
      : undefined,
    minRestHours: process.env.MIN_REST_HOURS
      ? Number(process.env.MIN_REST_HOURS)
      : undefined,
    sleeper_berth_split_long: process.env.SLEEPER_BERTH_SPLIT_LONG
      ? Number(process.env.SLEEPER_BERTH_SPLIT_LONG)
      : undefined,
